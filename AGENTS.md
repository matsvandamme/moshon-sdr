...
- [x] **M2.3** CW decoder. JS-side decoder in `lib/dsp/cw-decoder.ts` runs in the DSP worker whenever mode = CW: rolling 5 ms RMS → leaky peak/floor adaptive threshold → key-down/key-up timing → ITU morse lookup. Self-calibrating dit length (15 WPM seed, tracks observed dits with EMA). Decoded text streams to main via `cwText` messages and appears in a CW-only UI panel under the spectrum, complete with WPM readout and a Clear button.
- [x] **M2.4** RDS decoder. Rust `RdsDecoder` embedded in `WfmDemod`: 57 kHz coherent reference recovered by pilot tripling (pilot × ref38 → biquad-BPF at 57 kHz); fractional symbol clock at 2 × 1187.5 baud; biphase + differential decode; 26-bit sliding window with syndrome check against RDS offsets A/B/C/C'/D; group parser fills the 8-char PS (Group 0A) and 64-char RT (Group 2A) buffers. Worker polls the decoder at 2 Hz and posts to main only on change. UI panel shows PI (hex), PS, RT under the spectrum whenever mode = WFM AND lock is established. **Status: needs more hardware iteration** — first on-air test (Studio Brussel 102.1 MHz) showed intermittent lock but no PS/PI; three fixes shipped (phase flip, sync confirmation, immediate drop on miss). Retest pending.
- [x] **M2.5** HackRF One driver. Native WebUSB protocol implementation in `lib/usb/hackrf-protocol.ts` (vendor control transfers for sample rate, frequency, gain stages, baseband filter, transceiver mode). New `web/src/workers/hackrf-worker.ts` handles bulk-IN transfers on endpoint 0x81, repacks signed-int8 IQ to offset-binary u8 in a reused scratch buffer, and writes to the same SAB ring shape the RTL-SDR path produces — so the DSP worker is agnostic to which dongle is feeding it. `HackRfSource` mirrors `RtlSdrSource`'s public surface. App.svelte input-mode picker now has three tabs (RTL-SDR / HackRF / Network).
- [x] **M2.5b** HackRF per-stage gain controls. Three independent UI controls per the official docs: RF amp toggle (+11 dB), IF/LNA slider (0..40 dB step 8), BB/VGA slider (0..62 dB step 2). State persists in localStorage; changes apply live to the device without restarting the stream. The top-row Gain readout shows the three-value summary when in HackRF mode.
- [x] **M2.6** ADS-B Mode S extended squitter decoder. Rust `AdsbDemod` runs envelope + preamble correlation + PPM bit slicer + Mode S CRC-24 (poly 0x1FFF409) over the 2.4 MS/s IQ stream; emits DF17/18 frames as JSON. JS-side parser decodes the ME field type codes 1–4 (callsign), 9–18 (airborne position with CPR globally-unambiguous decode), 19 (ground velocity + heading + vertical rate). Aircraft tracker stores per-ICAO state with 60 s stale eviction. UI panel lists hex/callsign/position/altitude/speed/age, most-recent first. Auto-tunes to 1090 MHz on mode entry. Two new Rust tests cover the canonical pyModeS frame end-to-end.
- [x] **M2.8** Offset tuning + advanced device controls. Software offset
  tuning via NCO shift in every source worker (RTL-SDR, HackRF, network) —
  trig-recurrence NCO is ~2 % CPU at 2.4 MS/s with periodic resync. RTL-SDR
  advanced: PPM correction, bias-T toggle, direct-sampling mode (off / I / Q
  branch). HackRF advanced: antenna-power toggle, baseband-filter override.
  Collapsible Advanced panel in the UI, all values persist to localStorage
  and apply live to the device without restarting the stream.
- [x] **M2.7** LoRa monitor mode (partial). Spectrum-only at EU868 ch0 (868.1 MHz). The DSP worker treats `lora` mode like `adsb` — no audio path, FFT stays live for the user to see chirp activity. UI panel shows a simple channel-power "activity" indicator and is explicit that full CSS de-chirp + symbol decode + payload parse is multi-week work scheduled for a future release.
- [ ] HackRF One driver (legacy entry — see M2.5)
- [ ] ADS-B mode (legacy entry — see M2.6)
- [ ] RDS for WFM
- [ ] Audio + IQ recording
- [ ] CW filter + decoder
